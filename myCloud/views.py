    ...
    if request.method == "POST":
        ...
        try:
            ...
            if objs:
                ...
                result = [{'state': '0', 'info': '检索成功!'}, data]
            elif models.FileInfo.objects.filter(fId=fId, fUserId=user_id):
                result = [{'state': '0', 'info': '文件夹为空!'}]
            elif fId == 0:
                result = [{'state': '0', 'info': '您还没有上传过文件!'}]
            else:
                result = [{'state': '-211', 'info': '文件检索错误!'}]
    ...
    if request.method == "POST":
        try:
            ...
            if models.FileInfo.objects.filter(fName=fileInfo.fName,
                ...
                result = {'state': '-212', 'info': '文件上传失败，有同名文件！'}
            else:
        ...
        try:
            ...
            if fName:  # 新的文件名不能为空
                ...
                if obj.fUserId == get_userid(request):  # 判断文件是否属于该用户
                    obj.fName = fName
                    obj.save()
                    result = {'state': '0', 'info': '文件名更改成功！'}
                else:
                    result = {'state': '-221', 'info': '您没有访问该文件的权限！'}
            else:
                result = {'state': '-213', 'info': '文件名不能为空！'}
        except Exception as err:
            print(err)
            result = {'state': '-4', 'info': '服务器错误！'}
    ...
    if request.method == "POST":
        ...
        if (models.FileInfo.objects.filter(fId=fFolderId) or fFolderId == 0) and len(fIdList):  # 检索文件夹信息（判断该文件夹是否存在）
            ...
            if count == len(fIdList):
                result = {'state': '0', 'info': '文件移动成功！'}
            elif count > 0:
                result = {'state': '-214', 'info': '部分文件移动失败！'}
            else:
                result = {'state': '-215', 'info': '全部文件移动失败！'}
        else:
            result = {'state': '-216', 'info': '参数错误！'}
        return JsonResponse(result, safe=False)
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if obj.fUserId == get_userid(request):  # 判断文件是否属于该用户
                ...
                result = [{'state': '0', 'info': '文件名更改成功！'}, data]
            else:
                result = {'state': '-221', 'info': '您没有访问该文件的权限！'}
        except Exception as err:
            print(err)
